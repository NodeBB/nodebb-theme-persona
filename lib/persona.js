...
$(document).ready(function() {
	...
	function setupNProgress() {
		...
		$(window).on('action:ajaxify.end', function(ev, data) {
			...
			} else if (data.url.match('^unread')) {
				setupPersonatronCategorySelector();
	...
	function setupPersonatronCategorySelector() {
		$('.category-menu a').on('touchstart', function(e) {
			e.stopPropagation();
		}).on('click', function() {
			var pos = $('.category-menu').scrollLeft();
			$(window).one('action:ajaxify.end', function(ev, data) {
				if (data.url.match('^unread')) {
					$('.category-menu').scrollLeft(pos);
				}
			});
		});
	}
