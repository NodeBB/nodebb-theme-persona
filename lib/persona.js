...
$(document).ready(function() {
	...
	$(window).on('action:app.loggedIn', function (e, data) {
		app.parseAndTranslate('partials/sidebar', data.header, function (html) {
	...
	function setupMobileMenu() {
		...
		require(['pulling', 'storage'], function (Pulling, Storage) {
			...
			// prepare by emptying chats list
			require(['components'], function(components) {
				components.get('chats/list').empty();
			});
