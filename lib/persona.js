...
$(document).ready(function() {
	...
	function setupEditedByIcon() {
		function activateEditedTooltips() {
			$('[data-pid] [data-component="post/editor"]').each(function() {
				var el = $(this), icon;
		...
		$(window).on('action:posts.edited', function(ev, data) {
			...
			var el = parent.find('[data-component="post/editor"]').first();
			icon.prop('title', el.text()).tooltip('fixTitle').removeClass('hidden');
		});
	...
	function setupMobileMenu() {
		...
		require(['pulling', 'storage'], function (Pulling, Storage) {
			...
			$(window).on('resize action:ajaxify.start', function () {
				navSlideout.close();
				if (chatsSlideout) { chatsSlideout.close(); }
				$('.account .cover').css('top', $('[data-component="navbar"]').height());
			});
			...
			socket.on('event:user_status_change', function(data) {
				if (parseInt(data.uid, 10) === app.user.uid) {
					app.updateUserStatus($('#menu [data-component="user/status"]'), data.status);
					navSlideout.close();
				}
			});
			...
			function loadChats() {
				require(['forum/header/chat'], function (chat) {
	...
	function setupHoverCards() {
		require(['components'], function(components) {
			components.get('topic')
				.on('click', '[data-component="user/picture"],[data-component="user/status"]', generateUserCard);
		});
	...
	function setupFavouriteButtonOnProfile() {
		setupFavouriteMorph($('[data-component="account/cover"]'), ajaxify.data.uid, ajaxify.data.username);
	}
